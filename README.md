...
```javascript
const fs = require("fs");
const { VGM, VGMDataStream, VGMWriteDataCommand, VGMWaitWordCommand, VGMEndCommand } = require("vgm-parser");
...
stream.push(new VGMWriteDataCommand({ target: ym2413, addr: 48, data: 16 })); // set voice=@1 volume=0(max)
stream.markLoopPoint(); // mark here as loop point
stream.push(new VGMWriteDataCommand({ target: ym2413, addr: 16, data: 172 })); // set f-number=172
